...
using namespace std;
  double Roll_Array[15] = {};
  double Pitch_Array[15] = {};
  double Yaw_Array[15] = {};
...
if (pose.empty())
   ...
   br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "camera_link", "camera_pose"));
	...
	{ 
		...
		sum_yaw = sum_yaw +Yaw_Array[i];
		

     		Roll_Array[i] = Roll_Array[i+1];
		Pitch_Array[i] = Pitch_Array[i+1];
		Yaw_Array[i] = Yaw_Array[i+1];
		
		
	}
   

    Roll_Array[14]=yaw_true;
    Pitch_Array[14]=roll_true;
    Yaw_Array[14]=pitch_true;

   sum_roll += Roll_Array[14];
   sum_pitch += Pitch_Array[14];
   sum_yaw += Yaw_Array[14];
   



   orientation.roll = sum_pitch*180/M_PI/15;
   orientation.pitch = sum_roll*180/M_PI/15;
   orientation.yaw =  -sum_yaw*180/M_PI/15;
