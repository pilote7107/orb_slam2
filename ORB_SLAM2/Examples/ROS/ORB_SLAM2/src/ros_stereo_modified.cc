...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    if(do_rectify)
    {
        ...
        const tf::Matrix3x3 rotation270degXZ(   0, 1, 0,
                                                0, 0, 1,
                                                1, 0, 0);
        const tf::Matrix3x3 rotation90degY(   0, 0, 1,
                                               0, 1, 0,
                                               1, 0, 0);
        const tf::Matrix3x3 R = rotation270degXZ * rotation90degY;

        static tf::TransformBroadcaster br;

        tf::Matrix3x3 globalRotation_rh = cameraRotation_rh* R;
        tf::Vector3 globalTranslation_rh = cameraTranslation_rh* R;
   ...
   ROS_ERROR("ROLL: %f, PITCH: %f, YAW: %f", yaw*180/M_PI,roll*180/M_PI,pitch*180/M_PI);

   transform.setRotation(tf::createQuaternionFromRPY(yaw, roll, pitch));

   br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "camera_link", "camera_pose"));

   orientation.roll = yaw;
   orientation.pitch = roll;
   orientation.yaw = pitch;
   orientation_publisher.pub (orientation);
