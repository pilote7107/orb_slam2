...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::Image> left_sub(nh, "/tara/stereo/left/image_rect", 1);
    message_filters::Subscriber<sensor_msgs::Image> right_sub(nh, "/tara/stereo/right/image_rect", 1);
...
if (pose.empty())
   ...
   ROS_INFO("***before for Yaw_Vector SIZE = %i*** ",Yaw_Vector.size());





	for(int i=0;i<Roll_Vector.size()-1;i++)
	{ 
	...
	for(int i=0;i<Yaw_Vector.size()-1;i++)
	{ 
		sum_yaw = sum_yaw +Yaw_Vector.at(i);
		Yaw_Vector.at(i) = Yaw_Vector.at(i+1);
	}
   
    ROS_INFO("***add to Roll:=%f ***",roll_true);
    Roll_Vector.at(Roll_Vector.size()-1)=roll_true;
    sum_roll +=  Roll_Vector.at(Roll_Vector.size()-1);

    ROS_INFO("***add to Pitch:=%f ***",yaw_true);
    Pitch_Vector.at(Pitch_Vector.size()-1)=yaw_true;
    sum_pitch +=  Pitch_Vector.at(Pitch_Vector.size()-1);

    ROS_INFO("***add to Yaw:=%f ***",pitch_true);
...
ROS_INFO("***before meaning*** ");
   ...
   orientation.roll =  sum_roll*180.0/M_PI/Roll_Vector.size();

   orientation.pitch = -sum_pitch*180.0/M_PI/Pitch_Vector.size();

   orientation.yaw =   sum_yaw*180.0/M_PI/Yaw_Vector.size();
